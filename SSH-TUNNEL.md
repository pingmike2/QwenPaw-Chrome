...
## 环境事实（已确认）
- VPS: REDACTED_VPS_IP, SSH 端口 **REDACTED_SSH_PORT**, root 登录
...
**frp 隧道段在 VPS 内部完成**（frpc 连 frps 都是 127.0.0.1），不经过公网 → 不触发 DPI！只有 ssh -R 的 REDACTED_SSH_PORT 走公网（SSH 端口已被信任，不会被 DPI 拦）。
...
ssh -i /root/.ssh/vps-key-1 \
  ...
  -p REDACTED_SSH_PORT root@REDACTED_VPS_IP &
echo $! > /tmp/ssh-tunnel.pid
sleep 3

# 验证 1：VPS 本机（应 307 登录页重定向）
ssh -i /root/.ssh/vps-key-1 -p REDACTED_SSH_PORT root@REDACTED_VPS_IP \
  'curl -sS -o /dev/null -w "VPS本机HTTP:%{http_code}\n" http://127.0.0.1:50596/'

# 验证 2：公网（应 307）
curl -sS -o /dev/null -w "公网HTTP:%{http_code}\n" http://REDACTED_VPS_IP:50596/

# 验证 3：登录页内容
curl -sS -L http://REDACTED_VPS_IP:50596/ | grep -o '<title>[^<]*</title>'
...
```ini
[program:ssh-tunnel]
command=/usr/bin/ssh -i /root/.ssh/vps-key-1 -o ServerAliveInterval=30 -o ServerAliveCountMax=3 -o ExitOnForwardFailure=yes -o StrictHostKeyChecking=no -N -R 127.0.0.1:50596:127.0.0.1:8080 -p REDACTED_SSH_PORT root@REDACTED_VPS_IP
autostart=true
autorestart=true
startsecs=3
priority=55
...
浏览器打开 `http://REDACTED_VPS_IP:50596/` → 登录页 → `qwenpaw` / `REDACTED_PASSWORD` → Chromium 桌面。
